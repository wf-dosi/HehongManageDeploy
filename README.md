...
| 首次启动 | `bash deploy.sh` | 启动中间件、调用后端部署、查看 Web 日志、等待 nginx 运行、设置前端目录权限 | 前后端进入运行阶段 |
...
3. 执行 `docker compose up -d --build --wait --wait-timeout 300 nginx`，等待 nginx 容器进入运行状态，最长等待 300 秒。当前 nginx 未配置健康检查，此处不检查 HTTP 页面是否可访问；命令失败时停止部署。
4. nginx 容器运行后，执行 `chmod -R 755 HeHongfrontend`，将前端目录及其所有子目录和文件的权限统一设为 `755`。
...
| 数据库与迁移目录 | 基于已有状态执行迁移 |
| 前端产物 | 使用现有 `HeHongfrontend/dist`，等待 nginx 容器运行后，递归将前端目录及文件设为 `755` |
